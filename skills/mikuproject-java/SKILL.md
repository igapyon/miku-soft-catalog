---
name: mikuproject-java
description: Use only when the user explicitly says `mikuproject`, `miku project`, `mikuku project`, or `mikuproject-java` for mikuproject-specific WBS workflows. This skill keeps the behavior of mikuproject-skills where possible and swaps the runtime to the Java CLI.
---
...
- the user writes `mikuproject-java`
...
- treat `XML`, `workbook JSON`, and `patch JSON` as the canonical data forms

## Operations

Prefer the reusable APIs or documented runtime flows exposed for `mikuproject-java`.
Use these before falling back to direct file reads or UI-oriented flows.

## Runtime Discipline

For explicit `mikuproject` import/export or report requests, first check the `mikuproject-java` runtime before broad workspace exploration or generic tool discovery.

Use this order:

1. read this `SKILL.md`
2. check the documented Java runtime first
3. use the documented Java CLI/runtime flow if present
4. only if that path is missing or unusable, search for alternatives

For this repository:

- prefer the documented `mikuproject-java` runtime path
- do not search broadly through the workspace before checking the Java runtime path
- do not conclude that runtime dependencies are missing until the documented project-local path has also been checked when applicable

## Error Handling

Treat invalid document kind, missing base state for patch application, and missing required structure as hard errors.
Treat runtime warnings, partial imports, and export simplifications as soft errors.
